...
const CategoryDisplay = ({products,categories,catFilter,proFilter,brandId}) => {
  return (
      ...
      <div className="card border-0 rounded-0">
        <StyleCategoris className="card-body">
          <ul className="nav mx-auto d-flex flex-column">
            {
              catFilter(categories,proFilter(products,brandId)).map((i,key) => (
...
export default BrandProfileHoc(CategoryDisplay)
